...
;; The image should use at most 256 tiles since there's no mid-frame address
;; mode switching being done to get more tiles.
;;
...
;; the byte index of where to start the data.
...
.tiledatacopyloop
  ...
  ;; All the copying is done and LCDC is setup.
  ;; Nothing to do now so just halt and let the gameboy display the image.
  halt

SECTION "constants", ROM0
TileByteCount:
  ;; Uses 360 tiles because it was copied from another file that supports 360
  ;; tiles. This should probably be changed to 256 since the code doesn't
  ;; support more than that many tiles.
  DW 360 * 16
